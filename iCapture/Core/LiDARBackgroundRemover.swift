...
class LiDARBackgroundRemover: ObservableObject {
    ...
    private let processingQueue = DispatchQueue(label: "lidar.background.removal.queue", qos: .userInitiated)
    
    // LiDAR configuration
    private let vehicleDepthRange: ClosedRange<Float> = 1.0...10.0 // 1-10 meters
    private let backgroundDepthThreshold: Float = 15.0 // 15+ meters is background
    
    // MARK: - Public Interface
    
    func removeBackgroundWithLiDAR(
        image: UIImage,
        ...
        processingQueue.async { [weak self] in
            self?.performLiDARBackgroundRemoval(
                image: image,
                depthData: depthData
            ) { result in
                Task { @MainActor in
    ...
    func removeBackgroundFromPhotoDataWithLiDAR(
        imageData: Data,
        depthData: ARDepthData,
        ...
        removeBackgroundWithLiDAR(image: image, depthData: depthData) { processedImage in
            guard let processedImage = processedImage else {
                completion(nil)
                return
            }
            
            // Convert to data with HEIF/JPEG fallback
    ...
    private func performLiDARBackgroundRemoval(
        image: UIImage,
        depthData: ARDepthData,
        completion: @escaping (UIImage?) -> Void
    ) {
