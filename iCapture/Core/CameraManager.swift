...
extension CameraManager {
        ...
        sessionQueue.async { [weak self] in
            ...
            if self.captureSession.canAddOutput(self.photoOutput) {
                ...
                if self.photoOutput.isDepthDataDeliverySupported {
                    ...
                    print("CameraManager: Depth data delivery supported; defaulting to disabled for full resolution")
                } else {
            ...
            let movieFileOutput = DispatchQueue.main.sync {
                self.videoRecordingManager.getMovieFileOutput()
            }
    ...
    private func logSupportedFormats(for device: AVCaptureDevice) {
        ...
        for format in device.formats {
            ...
            print(
                """
                CameraManager: - Format: \(format) | video=\(dimensions.width)x\(dimensions.height) | \
                photoOptions=\(photoDimensionsDescription)
                """
            )
        }
    }
    ...
    func capturePhoto(triggerType: TriggerType) {
        ...
        sessionQueue.async { [weak self] in
            ...
            // Always prefer highest still photo resolution; disable per-capture depth delivery to avoid downscaling.
            if self.photoOutput.isDepthDataDeliveryEnabled {
                self.photoOutput.isDepthDataDeliveryEnabled = false
                print("CameraManager: Disabled depth data delivery for this capture to maximize resolution")
            }
            ...
            if #available(iOS 16.0, *) {
                ...
                if let preferred = self.preferredPhotoDimensions {
                    let currentMax = self.photoOutput.maxPhotoDimensions
                    let currentArea = Int(currentMax.width) * Int(currentMax.height)
                    let preferredArea = Int(preferred.width) * Int(preferred.height)
                    if currentArea == 0 || preferredArea > currentArea {
                        self.photoOutput.maxPhotoDimensions = preferred
                        print(
                            "CameraManager: Updated photo output maxPhotoDimensions to \(preferred.width)x\(preferred.height)"
                        )
                    }
                    ...
                    print(
                        """
                        CameraManager: Requested photo dimensions: \(preferred.width)x\(preferred.height) \
                ...
                print(
                    """
                    CameraManager: Active format high-resolution dimensions: \(highRes.width)x\(highRes.height) \
                    (~\(String(format: "%.1f", megapixels))MP)
                    """
                )
            }
